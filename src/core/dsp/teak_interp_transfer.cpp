...
#define EXCH_FUNC(name, i, j, ops, op0s, op1s, op2s) int TeakInterp::name(uint16_t opcode) { \
    ...
    dsp.writeData(stepReg(arpR##i[(opcode >> ops) & 0x3], arpP##i[(opcode >> op1s) & 0x3]), value); \
    value = dsp.readData(stepReg(arpR##j[(opcode >> ops) & 0x3], arpP##j[(opcode >> op2s) & 0x3])); \
...
MOV_FUNC(movMi8sv, dsp.readData((regMod[1] << 8) | (opcode & 0xFF)), regSv=, 1) // MOV MemImm8, SV
MOV_FUNC(movMrnr6, dsp.readData(getRnStepZids(opcode)), regR[6]=, 1) // MOV MemRnStepZids, R6
...
#define MOVAM_FUNC(name, op0, ars0, ars1) int TeakInterp::name(uint16_t opcode) { \
    ...
    dsp.writeData(getRarOffsAr(rar), value >> 0); \
    dsp.writeData(getRarStepAr(rar), value >> 16); \
    return 1; \
}
...
#define MOVMA_FUNC(name, ars0, ars1, op1) int TeakInterp::name(uint16_t opcode) { \
    uint8_t rar = ((opcode >> ars0) & 0xC) | ((opcode >> ars1) & 0x3); \
    uint16_t l = dsp.readData(getRarOffsAr(rar)); \
    uint16_t h = dsp.readData(getRarStepAr(rar)); \
...
int TeakInterp::mov2Abhabh(uint16_t opcode) { // MOV2 Abh, Abh, MemRar1StepAr1
    ...
    dsp.writeData(getRarOffsAr(rar), (this->*readAbS[(opcode >> 2) & 0x3])() >> 16);
    dsp.writeData(getRarStepAr(rar), (this->*readAbS[(opcode >> 4) & 0x3])() >> 16);
    return 1;
}

// Pop a value from the stack
#define POP_FUNC(name, op0) int TeakInterp::name(uint16_t opcode) { \
    op0(dsp.readData(regSp++)); \
    return 1; \
}
...
int TeakInterp::popAbe(uint16_t opcode) { // POP Abe
    ...
    int64_t value = (int64_t(dsp.readData(regSp++)) << 32) | (*readAb[opcode & 0x3] & 0xFFFFFFFF);
...
#define POPA_FUNC(name, op0) int TeakInterp::name(uint16_t opcode) { \
    uint16_t h = dsp.readData(regSp++); \
    uint16_t l = dsp.readData(regSp++); \
...
#define PUSHA_FUNC(name, op0) int TeakInterp::name(uint16_t opcode) { \
    int64_t value = op0; \
    dsp.writeData(--regSp, value >> 0); \
    dsp.writeData(--regSp, value >> 16); \
    return 1; \
}
