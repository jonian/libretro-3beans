...
ADD16_FUNC(addMi16a, dsp.readData(readParam()), 8, 2) // ADD MemImm16, Ax
ADD16_FUNC(addMi8a, dsp.readData((regMod[1] << 8) | (opcode & 0xFF)), 8, 1) // ADD MemImm8, Ax
ADD16_FUNC(addM7i16a, dsp.readData(regR[7] + readParam()), 8, 2) // ADD MemR7Imm16, Ax
ADD16_FUNC(addM7i7a, dsp.readData(regR[7] + (int8_t(opcode << 1) >> 1)), 8, 1) // ADD MemR7Imm7s, Ax
ADD16_FUNC(addMrna, dsp.readData(getRnStepZids(opcode)), 8, 1) // ADD MemRnStepZids, Ax
...
ADDH_FUNC(addhMi8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF)), 8) // ADDH MemImm8, Ax
ADDH_FUNC(addhMrn, dsp.readData(getRnStepZids(opcode)), 8) // ADDH MemRnStepZids, Ax
...
ADDL_FUNC(addlMi8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF)), 8) // ADDL MemImm8, Ax
ADDL_FUNC(addlMrn, dsp.readData(getRnStepZids(opcode)), 8) // ADDL MemRnStepZids, Ax
...
AND_FUNC(andMi16, dsp.readData(readParam()), 8, 2) // AND MemImm16, Ax
AND_FUNC(andMi8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF)), 8, 1) // AND MemImm8, Ax
AND_FUNC(andM7i16, dsp.readData(regR[7] + readParam()), 8, 2) // AND MemR7Imm16, Ax
AND_FUNC(andM7i7, dsp.readData(regR[7] + (int8_t(opcode << 1) >> 1)), 8, 1) // AND MemR7Imm7s, Ax
AND_FUNC(andMrn, dsp.readData(getRnStepZids(opcode)), 8, 1) // AND MemRnStepZids, Ax
...
#define CHNGM_FUNC(name, op1a) int TeakInterp::name(uint16_t opcode) { \
    uint16_t addr = op1a; \
    uint16_t res = dsp.readData(addr) ^ readParam(); \
    bool z = (res == 0); \
    bool m = (res & BIT(15)); \
...
CMP16_FUNC(cmpMi16a, dsp.readData(readParam()), 8, 2) // CMP MemImm16, Ax
CMP16_FUNC(cmpMi8a, dsp.readData((regMod[1] << 8) | (opcode & 0xFF)), 8, 1) // CMP MemImm8, Ax
CMP16_FUNC(cmpM7i16a, dsp.readData(regR[7] + readParam()), 8, 2) // CMP MemR7Imm16, Ax
CMP16_FUNC(cmpM7i7a, dsp.readData(regR[7] + (int8_t(opcode << 1) >> 1)), 8, 1) // CMP MemR7Imm7s, Ax
CMP16_FUNC(cmpMrna, dsp.readData(getRnStepZids(opcode)), 8, 1) // CMP MemRnStepZids, Ax
...
CMPU_FUNC(cmpuMi8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF)), 8) // CMPU MemImm8, Ax
CMPU_FUNC(cmpuMrn, dsp.readData(getRnStepZids(opcode)), 8) // CMPU MemRnStepZids, Ax
...
CMPV_FUNC(cmpvMi8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF))) // CMPV Imm16, MemImm8
CMPV_FUNC(cmpvMrn, dsp.readData(getRnStepZids(opcode))) // CMPV Imm16, MemRnStepZids
...
EXP_FUNC(expMrn, int32_t(dsp.readData(getRnStepZids(opcode)) << 16)) // EXP MemRnStepZids
...
EXPA_FUNC(expMrna, int32_t(dsp.readData(getRnStepZids(opcode)) << 16), 8) // EXP MemRnStepZids, Ax
...
#define MAA_FUNC(name, xu, yu, op0a, op1, cyc) int TeakInterp::name(uint16_t opcode) { \
    ...
    bool c = (uint64_t(val1) > uint64_t(res)); \
    regX[0] = op1; \
    regY[0] = dsp.readData(getRnStepZids(op0a)); \
    writeStt0((regStt[0] & ~0xFC) | calcZmne(res) | (v << 4) | (c << 3) | (v << 1)); \
...
MAA_FUNC(maaMrmr, ,, ((opcode >> 2) & 0x19) + 4, dsp.readData(getRnStepZids(opcode & 0x1B)), 1) // MAA MR, MR, Ax
MAA_FUNC(maaMrni16, ,, (opcode & 0x1F), readParam(), 2) // MAA MemRnStepZids, Imm16, Ax
MAA_FUNC(maasuMrmr, u,, ((opcode >> 2) & 0x19) + 4, dsp.readData(getRnStepZids(opcode & 0x1B)), 1) // MAASU M,M,Ax
...
MAAY_FUNC(maaY0mi8, ,, dsp.readData((regMod[1] << 8) | (opcode & 0xFF)), 11) // MAA Y0, MemImm8, Ax
MAAY_FUNC(maaY0mrn, ,, dsp.readData(getRnStepZids(opcode)), 11) // MAA Y0, MemRnStepZids, Ax
MAAY_FUNC(maaY0reg, ,, *readReg[opcode & 0x1F], 11) // MAA Y0, Register, Ax
MAAY_FUNC(maaY0r6, ,, regR[6], 0) // MAA Y0, R6, Ax
MAAY_FUNC(maasuY0mrn, u,, dsp.readData(getRnStepZids(opcode)), 11) // MAASU Y0, MemRnStepZids, Ax
...
MAC_FUNC(macMrmr, ,, ((opcode >> 2) & 0x19) + 4, dsp.readData(getRnStepZids(opcode & 0x1B)), 1) // MAC MR, MR, Ax
MAC_FUNC(macMrni16, ,, (opcode & 0x1F), readParam(), 2) // MAC MemRnStepZids, Imm16, Ax
MAC_FUNC(macsuMrmr, u,, ((opcode >> 2) & 0x19) + 4, dsp.readData(getRnStepZids(opcode & 0x1B)), 1) // MACSU M,M,Ax
MAC_FUNC(macsuMrni16, u,, (opcode & 0x1F), readParam(), 2) // MACSU MemRnStepZids, Imm16, Ax
MAC_FUNC(macusMrmr, ,u, ((opcode >> 2) & 0x19) + 4, dsp.readData(getRnStepZids(opcode & 0x1B)), 1) // MACUS M,M,Ax
MAC_FUNC(macusMrni16, ,u, (opcode & 0x1F), readParam(), 2) // MACUS MemRnStepZids, Imm16, Ax
MAC_FUNC(macuuMrmr, u,u, ((opcode >> 2) & 0x19) + 4, dsp.readData(getRnStepZids(opcode & 0x1B)), 1) // MACUU M,M,A
...
MACY_FUNC(macY0mi8, ,, dsp.readData((regMod[1] << 8) | (opcode & 0xFF)), 11) // MAC Y0, MemImm8, Ax
MACY_FUNC(macY0mrn, ,, dsp.readData(getRnStepZids(opcode)), 11) // MAC Y0, MemRnStepZids, Ax
MACY_FUNC(macY0reg, ,, *readReg[opcode & 0x1F], 11) // MAC Y0, Register, Ax
MACY_FUNC(macY0r6, ,, regR[6], 0) // MAC Y0, R6, Ax
MACY_FUNC(macsuY0mi8, u,, dsp.readData((regMod[1] << 8) | (opcode & 0xFF)), 11) // MACSU Y0, MemImm8, Ax
MACY_FUNC(macsuY0mrn, u,, dsp.readData(getRnStepZids(opcode)), 11) // MACSU Y0, MemRnStepZids, Ax
MACY_FUNC(macsuY0reg, u,, *readReg[opcode & 0x1F], 11) // MACSU Y0, Register, Ax
MACY_FUNC(macsuY0r6, u,, regR[6], 0) // MACSU Y0, R6, Ax
MACY_FUNC(macusY0mrn, ,u, dsp.readData(getRnStepZids(opcode)), 11) // MACUS Y0, MemRnStepZids, Ax
MACY_FUNC(macusY0reg, ,u, *readReg[opcode & 0x1F], 11) // MACUS Y0, Register, Ax
MACY_FUNC(macusY0r6, ,u, regR[6], 0) // MACUS Y0, R6, Ax
MACY_FUNC(macuuY0mrn, u,u, dsp.readData(getRnStepZids(opcode)), 11) // MACUU Y0, MemRnStepZids, Ax
...
#define MMA_FUNC(n, b, p0a, p1a, x0u, y0u, x1u, y1u, ops, opm, op0s, op1s, op2s) int TeakInterp::n(uint16_t opcode) { \
    ...
    bool v = (~(val2 ^ val1) & (res ^ val2)) >> 39; \
    bool c = (uint64_t(val1) > uint64_t(res)); \
    regX[1] = dsp.readData(offsReg(arpRi[(opcode >> ops) & opm], arpCi[(opcode >> op1s) & opm])); \
    regY[1] = dsp.readData(offsReg(arpRj[(opcode >> ops) & opm], arpCj[(opcode >> op0s) & opm])); \
    regX[0] = dsp.readData(stepReg(arpRi[(opcode >> ops) & opm], arpPi[(opcode >> op1s) & opm])); \
    regY[0] = dsp.readData(stepReg(arpRj[(opcode >> ops) & opm], arpPj[(opcode >> op0s) & opm])); \
    writeStt0((regStt[0] & ~0xFC) | calcZmne(res) | (v << 4) | (c << 3) | (v << 1)); \
...
#define MMAY_FUNC(name, p1a, x0u, y0u, x1u, y1u, ars0, ars1, op1s) int TeakInterp::name(uint16_t opcode) { \
    ...
    regX[1] = dsp.readData(getRarOffsAr(rar)); \
    regX[0] = dsp.readData(getRarStepAr(rar)); \
    writeStt0((regStt[0] & ~0xFC) | calcZmne(res) | (v << 4) | (c << 3) | (v << 1)); \
    (this->*writeAx40S[(opcode >> op1s) & 0x1])(res); \
    multiplyXY<x1u##int16_t, y1u##int16_t>(1); \
    multiplyXY<x0u##int16_t, y0u##int16_t>(0); \
...
#define MOVSM_FUNC(name, op0a, op1s) int TeakInterp::name(uint16_t opcode) { \
    (this->*writeAb40[(opcode >> op1s) & 0x3])(shift(int16_t(dsp.readData(op0a)), regSv)); \
    return 1; \
}
...
MPY_FUNC(mpyY0mi8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF))) // MPY Y0, MemImm8
MPY_FUNC(mpyY0mrn, dsp.readData(getRnStepZids(opcode))) // MPY Y0, MemRnStepZids
...
MPYSU_FUNC(mpysuY0mrn, dsp.readData(getRnStepZids(opcode))) // MPYSU Y0, MemRnStepZids
...
int TeakInterp::mpysuMrmr(uint16_t opcode) { // MPYSU MemR45StepZids, MemR0123StepZids
    ...
    regX[0] = dsp.readData(getRnStepZids(opcode & 0x1B));
    regY[0] = dsp.readData(getRnStepZids(((opcode >> 2) & 0x19) + 4));
    multiplyXY<uint16_t, int16_t>(0);
    return 1;
}
...
MSU_FUNC(msuMrmr, ((opcode >> 2) & 0x19) + 4, dsp.readData(getRnStepZids(opcode & 0x1B)), 1) // MSU MR45, MR03, Ax
...
MSUY_FUNC(msuY0mi8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF)), 8) // MSU Y0, MemImm8, Ax
MSUY_FUNC(msuY0mrn, dsp.readData(getRnStepZids(opcode)), 8) // MSU Y0, MemRnStepZids, Ax
...
OR_FUNC(orMi16, dsp.readData(readParam()), regA, 8, 8, 2) // OR MemImm16, Ax
OR_FUNC(orMi8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF)), regA, 8, 8, 1) // OR MemImm8, Ax
OR_FUNC(orM7i16, dsp.readData(regR[7] + readParam()), regA, 8, 8, 2) // OR MemR7Imm16, Ax
OR_FUNC(orM7i7, dsp.readData(regR[7] + (int8_t(opcode << 1) >> 1)), regA, 8, 8, 1) // OR MemR7Imm7s, Ax
OR_FUNC(orMrn, dsp.readData(getRnStepZids(opcode)), regA, 8, 8, 1) // OR MemRnStepZids, Ax
...
SQR_FUNC(sqrMi8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF))) // SQR MemImm8
SQR_FUNC(sqrMrn, dsp.readData(getRnStepZids(opcode))) // SQR MemRnStepZids
...
SUB16_FUNC(subMi16a, dsp.readData(readParam()), 8, 2) // SUB MemImm16, Ax
SUB16_FUNC(subMi8a, dsp.readData((regMod[1] << 8) | (opcode & 0xFF)), 8, 1) // SUB MemImm8, Ax
SUB16_FUNC(subM7i16a, dsp.readData(regR[7] + readParam()), 8, 2) // SUB MemR7Imm16, Ax
SUB16_FUNC(subM7i7a, dsp.readData(regR[7] + (int8_t(opcode << 1) >> 1)), 8, 1) // SUB MemR7Imm7s, Ax
SUB16_FUNC(subMrna, dsp.readData(getRnStepZids(opcode)), 8, 1) // SUB MemRnStepZids, Ax
...
SUBH_FUNC(subhMi8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF))) // SUBH MemImm8, Ax
SUBH_FUNC(subhMrn, dsp.readData(getRnStepZids(opcode))) // SUBH MemRnStepZids, Ax
...
SUBL_FUNC(sublMi8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF))) // SUBL MemImm8, Ax
SUBL_FUNC(sublMrn, dsp.readData(getRnStepZids(opcode))) // SUBL MemRnStepZids, Ax
...
TSTB_FUNC(tstbMi8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF)), opcode >> 8, 1) // TSTB MemImm8, Imm4
TSTB_FUNC(tstbMrn, dsp.readData(getRnStepZids(opcode)), opcode >> 8, 1) // TSTB MemRnStepZids, Imm4
...
TST0A_FUNC(tst0Almi8, 8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF))) // TST0 Axl, MemImm8
...
TST0I_FUNC(tst0I16mi8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF))) // TST0 Imm16, MemImm8
...
TST1A_FUNC(tst1Almi8, 8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF))) // TST1 Axl, MemImm8
...
TST1I_FUNC(tst1I16mi8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF))) // TST1 Imm16, MemImm8
...
XOR_FUNC(xorMi16, dsp.readData(readParam()), 8, 2) // XOR MemImm16, Ax
XOR_FUNC(xorMi8, dsp.readData((regMod[1] << 8) | (opcode & 0xFF)), 8, 1) // XOR MemImm8, Ax
XOR_FUNC(xorM7i16, dsp.readData(regR[7] + readParam()), 8, 2) // XOR MemR7Imm16, Ax
XOR_FUNC(xorM7i7, dsp.readData(regR[7] + (int8_t(opcode << 1) >> 1)), 8, 1) // XOR MemR7Imm7s, Ax
XOR_FUNC(xorMrn, dsp.readData(getRnStepZids(opcode)), 8, 1) // XOR MemRnStepZids, Ax
