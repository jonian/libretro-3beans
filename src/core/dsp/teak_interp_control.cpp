...
#define BKREPRST_FUNC(name, op0) int TeakInterp::name(uint16_t opcode) { \
    uint16_t &reg = op0; \
    uint8_t count = (regStt[2] >> 12) & 0x7; \
    uint16_t ext = dsp.readData(reg++); \
    bkEnd[count] = ((ext << 8) & 0x10000) | dsp.readData(reg++); \
    bkStart[count] = ((ext << 16) & 0x10000) | dsp.readData(reg++); \
    regLc = dsp.readData(reg++); \
...
#define BKREPSTO_FUNC(name, op0) int TeakInterp::name(uint16_t opcode) { \
    ...
    dsp.writeData(--reg, regLc); \
    dsp.writeData(--reg, bkStart[count]); \
    dsp.writeData(--reg, bkEnd[count]); \
    uint16_t ext = (regStt[2] & BIT(15)) | ((bkEnd[count] >> 8) & 0x300) | ((bkStart[count] >> 16) & 0x3); \
    dsp.writeData(--reg, ext); \
    bkrepPop(count); \
    return 1; \
}
